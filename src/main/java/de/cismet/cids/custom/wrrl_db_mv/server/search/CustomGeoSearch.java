...
public class CustomGeoSearch extends AbstractCidsServerSearch implements GeoSearch {
    ...
    public String getSearchSql(final String domainKey) {
        ...
        String cidsSearchGeometryWKT = geometry.toText();
        ...
        if (geometry.getCoordinates().length > 1000) {
            LOG.warn("length of the geometry: " + geometry.getCoordinates().length + " "
                        + " Geometry will be simplified to a length of: "
                        + TopologyPreservingSimplifier.simplify(geometry, 30).getCoordinates().length);
            cidsSearchGeometryWKT = TopologyPreservingSimplifier.simplify(geometry, 30).toText();
        }
                ...
                if (sqlStatement != null) {
                    ...
                    for (final ArrayList al : result) {
                        ...
                        try {
                            ...
                            if (LOG.isDebugEnabled()) {
                                LOG.debug("no name present for metaobjectnode", e); // NOI18N
                            }
                        }
